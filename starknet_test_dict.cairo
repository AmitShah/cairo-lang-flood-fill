...
func gravity(column_len:felt, column : felt *, new_column_len:felt, new_column: felt*)->(c:felt):
    if column_len == -1:
...
func main{output_ptr : felt*, range_check_ptr}():
    ...
    let (xd) = fill_algorithm(7, 3, 1, board, x_dict )
     default_dict_finalize(
        dict_accesses_start=x_dict,
        dict_accesses_end=xd,
        default_value=0)
    ...
    serialize_word(new_column[0])    
    serialize_word(new_column[1])
    serialize_word(new_column[2])
    serialize_word(new_column[3])
    serialize_word(new_column[4])
    serialize_word(new_column[5])
    serialize_word(new_column[6])
     serialize_word(new_column[7])
    serialize_word(new_column[8])
    return ()
end
